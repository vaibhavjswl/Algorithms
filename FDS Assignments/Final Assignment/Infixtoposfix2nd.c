...
void main()
{
    ...
    char exit = 'n';
    int choice ;
    //char *prefix = malloc(MAX * (sizeof(char)));
...
char pop()
{
    ...
    {
       // if (stack.element[stack.top]!='(') printf("%c",stack.element[stack.top]);
        stack.top--;
         ...
         else
         {
             ...
             else if(Operatorcheck(infix[i])==1)
             {
                 ...
                 else
                 {
                     ...
                     else if (getPriority(infix[i])<=getPriority(displaytop()))          // here see if we need to do while loop to pop all ele with priority less str[i]
                     {
